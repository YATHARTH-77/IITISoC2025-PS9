...
print("Running final updated french.py - Version 2025-08-02 12:35 AM IST")  # Updated version marker
...
# Initialize OCR reader for French and English
...
# Load checkpoint to extract character list and model info
print("Reached checkpoint loading")  # Debug point
...
print(f"Character list: {character_list}")  # Debug character list
print(f"Length of character_list: {len(character_list)}")  # Should be 108

# FIXED: Get num_classes directly from the saved model weights
num_classes = checkpoint['model_state_dict']['classifier.bias'].shape[0]
print(f"Setting num_classes to {num_classes} from checkpoint")  # Should print 109

# Create charset for decoding (add blank token at index 0 if using CTC)
charset = [''] + character_list  # Add blank token at the beginning
print(f"Final charset length: {len(charset)}")  # Should match num_classes

# Instantiate the model with the correct num_classes
model = SimpleCRNN(num_classes=num_classes).to(device)
print(f"Model classifier weight shape: {model.classifier.weight.shape}")  # Should print [109, 512]
...
# Function to run OCR with both fine-tuned model and EasyOCR, with Tesseract fallback
def run_ocr_all(image_np, box_id):
    ...
    # Run EasyOCR on multiple processed images
    ...
    if conf_custom > threshold_custom:
        ...
        text_tesseract = pytesseract.image_to_string(resized, lang='fra+eng', config='--psm 6')
