...
print("Running updated korean_simultaneous.py - Version 2025-08-02 02:00 AM IST")  # Version marker
...
model_path = "../../Weights/Korean/best_korean_ocr_model.pth"    # Path to fine-tuned weights
character_list_path = "../../Weights/Korean/training_data/character_list.txt"  # Path to character list
...
print("EasyOCR reader initialized for languages: ko, en")
...
# Try to load Korean fine-tuned model
if os.path.exists(weights_path):
    try:
        print("Loading Korean fine-tuned model...")
        ...
        print("Korean fine-tuned model loaded successfully!")
        
    except Exception as e:
        print(f"Failed to load Korean fine-tuned model: {e}")
        print("Continuing with EasyOCR and Tesseract only...")
        model = None
else:
    print(f"Korean fine-tuned model not found at {weights_path}")
...
def run_ocr_all(image_np, box_id):
    ...
    img_tensor = preprocess(img_pil).unsqueeze(0).to(device) if preprocess is not None else None
    ...
    if conf_custom > threshold_custom:
        ...
        text_tesseract = pytesseract.image_to_string(resized, lang='kor+eng', config='--psm 6')
...
output_json_path = os.path.join(output_folder, "results_korean.json")
...
with open(output_json_path, 'w', encoding='utf-8') as f:
    json.dump({
        ...
        "korean_regions": sum(1 for result in output_results if result.get("contains_korean", False)),
        "texts": output_results
    }, f, ensure_ascii=False, indent=4)

print(f"Results saved to {output_json_path}")
print(f"Processed {len(output_results)} text regions")
korean_count = sum(1 for result in output_results if result.get("contains_korean", False))
print(f"Found {korean_count} regions containing Korean text")
if model is not None:
    print("✓ Fine-tuned Korean model was used as primary OCR method")
else:
