...
print("Running updated italian_simultaneous.py - Version 2025-08-02 01:54 AM IST")  # Version marker
...
model_path = "../../Weights/Italian/best_italian_ocr_model.pth"  # Path to fine-tuned weights
...
# Initialize OCR reader for Italian and English
reader = easyocr.Reader(['it', 'en'], gpu=torch.cuda.is_available())
print("EasyOCR reader initialized for languages: it, en")
...
if os.path.exists(model_path):
    try:
        print("Loading Italian fine-tuned model...")
        ...
        print("Italian fine-tuned model loaded successfully!")
...
def run_ocr_all(image_np, box_id):
    ...
    if conf_custom > threshold_custom:
        ...
        text_tesseract = pytesseract.image_to_string(resized, lang='ita+eng', config='--psm 6')
...
output_json_path = os.path.join(output_folder, "results_italian.json")
...
if model is not None:
    print("Used fine-tuned Italian model as primary OCR method")
else:
